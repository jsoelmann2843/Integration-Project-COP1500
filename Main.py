#Command Mage by Jarrett Oelmann
#This program is a text-based adventure game
#Anything that crashs the program while it runs is considered to be the end of the world and is a game over.
def mana(): #function so that I don't have to uses the longer print below
    print("The world now has ", world_mana, " mana.", sep='') #the sep='' makes it so that a comma is equal to nothing rather than the default of a space
...
# line.
if option_selection > "x":       #used to check for a yes or no anwser by comparing the values of ASKII code of string input
    world_mana = world_mana - 10    #subtracts the integar 10 from variable world_mana
    print("You summon rain!", end='')   #the end='' makes the next print function display the text on the same line as this print
    print(" drip." * 10)   #repeats the "sound" of rain 10 times on the line
    mana()
    world_mana = world_mana + 20   #adds the integar 20 to variable world_mana
    print("The world has more mana as a result of your actions!")
    print("The world has ", world_mana, " mana.", sep='')
else:
    print("You ignore" + " the village.")   #the + is used to add the picture of both strings
mana_used = float(input("You continue on and find a huge slime in your "
                        "way!\nHow much mana do you use to try and remove "
                        "it? "))  #asks for a input that comes in as a
# string but is converted to a float
if mana_used >= 3:            #if mana_used is greater than or equal to 3 then...
    world_mana = world_mana - mana_used     #subtract the mana_used from world_mana and asign that new value to world_mana
    if world_mana > 0:       #the whole point of this game is to stop world mana from running out so if world_mana is 0 or less it is game over
        print("You removed the slime!")
        mana()
    else:
        print("You caused the world to be destroyed by using the last of its "
              "mana.")
        world_end = 10 % "world"  # Don't know how to end a program so I just crash the code. The code crashing is the end of this world.
else:
    print("You fail to remove the slime!\nYou get absorbed into the "
          "slime!\nThe acid inside slowly disolves your flesh.\nYou "
...
puzzle_input = float(input("How much mana do you push into the door to solve "
                           "the puzzle? "))
world_mana = world_mana - puzzle_input
if world_mana > 0:    #if world mana is greater than 0
    mana()
else:
    print("You caused the world to be destroyed by using the last of its mana.")
    world_end = 10 % "world"  # Don't know how to end a program so I just crash the code. The code crashing is the end of this world.
puzzle_check = (((puzzle_input ** 2) + 1) / 5 ) * 1   #takes the input value assigned to puzzle_input and takes it to the power of 2 and then adds a 1 to it and then it divides by 5 and finally multiplies by 1 and assigns that to puzzle_check
if puzzle_check == 1:       #checks the variable to see if it is equal to 1
    print("The puzzle door open to a closed room with a single locked chest "
          "inside.")
    mana_used = float(input("How much mana do you use to open the chest? "))   #reassigns the mana_used variable
    world_mana = world_mana - mana_used
    if world_mana > 0:
        mana()
    else:
        print("You caused the world to be destroyed by using the last of its "
              "mana.")
        world_end = 10 % "world"  # Don't know how to end a program so I just crash the code. The code crashing is the end of this world.
    puzzle_check = mana_used % 2   #reassigns variable and used to find the remainder of input entered so I can check for even or odd
    if puzzle_check == 0:     #if variable is a 0 than number inputted is even if there is a one it is odd
        print("The chest opens!\nThere is some mana inside!")
        world_mana = (world_mana // 5) + world_mana   #take variable and uses floor division by 5 and adds that to world_mana to be assigned to be the new world_mana
        mana()
    else:
